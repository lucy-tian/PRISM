...
ldlc = pd.read_csv('LDL-C.grouping.tsv', sep='\t')

cols = [i for i in df.columns if 'liv' in i.lower() or 'hepg2' in i.lower()]
...
merged = all_df.merge(ldlc, left_on='SNP', right_on='rsID')
...
or_results.to_csv(f'{out_d}/LDL-C_table.tsv', sep='\t', index=False)
